...
SW3             cmpa    #SW3_VAL        ; Is SW2 presssed? bit 3 = 0 ?
                bne     SW4             ; No, get another SW value
                ldab    #PATTERN_3      ; Yes, so set up to display Switch #: 2 base 2
                jsr     Display_Value   ; to LEDs
                bra     Get_SW          ; check again for presseed switch

SW4             cmpa    #SW4_VAL        ; Is SW2 presssed? bit 3 = 0 ?
                bne     SW5             ; No, get another SW value
                ldab    #PATTERN_4      ; Yes, so set up to display Switch #: 2 base 2
                jsr     Display_Value   ; to LEDs
                bra     Get_SW          ; check again for presseed switch

SW5             cmpa    #SW5_VAL        ; Is SW2 presssed? bit 3 = 0 ?
                bne     LEDsOff         ; No, get another SW value
                ldab    #PATTERN_5      ; Yes, so set up to display Switch #: 2 base 2
                jsr     Display_Value   ; to LEDs
                bra     Get_SW          ; check again for presseed switch

Display_Value   stab    PORTB        ; Value to LEDs
